...
def main():
    ...
    if args.offline == True:
        if args.subset == "all":
            subsets = ["fr_emea", "fr_medline", "fr_patents"]
        else:
            subsets = [args.subset]

        dataset = None
        for subset in subsets:
            dataset_tmp = load_from_disk(
                f"{args.data_dir.rstrip('/')}/local_hf_{subset}/"
            )
            dataset_tmp = concatenate_datasets(
                [dataset_tmp["train"], dataset_tmp["validation"], dataset_tmp["test"]]
            )
            label_list = dataset_tmp.features[f"ner_tags"].feature.names
            none_label = label_list.index("O")

            def no_labels_filter(example):
                if np.all(np.array(example["ner_tags"]) == none_label):
                    return False
                return True

            dataset_tmp = dataset_tmp.filter(no_labels_filter)
            ...
            if dataset:
                ...
                new_id2label = {
                    ...
                    for k, v in enumerate(
                        dataset_tmp.features[f"ner_tags"].feature.names
                    )
                }
                new_label2id = {
                    v: k
                ...
                # label to remap because different mapping
                for l, i in label2id.items():
                    if l in new_label2id and new_label2id[l] != i:
                        # we want to map the inverse of v in new_labels_mapping to k
                        dataset_remap[new_label2id[l]] = i

                # label to add to mapping because new labels
                for l, i in new_label2id.items():
                    if l not in label2id:
                        dataset_remap[i] = len(label2id)
                        label2id[l] = len(label2id)
                        id2label[len(label2id)] = l
            ...
            if not dataset:
                ...
                # remap labels using dataset_remap
                new_class_label = ClassLabel(names=[l for i, l in id2label.items()])
                features = Features(
                    {
                        ...
                        "ner_tags": Sequence(
                            new_class_label
                        ),  # Use the new ClassLabel object
                    }
                )
                dataset_tmp = dataset_tmp.cast(features)

                dataset_tmp = dataset_tmp.map(
                    lambda x: {
                        "ner_tags": [dataset_remap.get(i, i) for i in x["ner_tags"]]
                    }
                )
                dataset = dataset.cast(features)
                dataset = concatenate_datasets([dataset, dataset_tmp])
    ...
    def tokenize_and_align_labels(examples):
        if args.model_name.lower().find("flaubert") != -1:
            ...
            for word_list in list(examples["tokens"]):
                # Convert the list to a tuple
                word_tuple = tuple(word_list)

                # Check if the tuple is already in the set
                if word_tuple in unique_lists:
                    # If it is, it's a duplicate
                    duplicates.append(word_list)
                else:
                    # Otherwise, add it to the set
                    unique_lists.add(word_tuple)
    ...
    dev_tokenized_datasets = dev_dataset.map(
        tokenize_and_align_labels, batched=True, keep_in_memory=True
    )
