...
def main():
    ...
    def is_unique(example):
        source_cible = example["source"] + example["cible"]
        cible_source = example["cible"] + example["source"]
        if source_cible in seen or cible_source in seen:
            return False
        seen.add(source_cible)
        ...
        res = {
            "input_ids": {
                ...
                "text_2_ids": tokenizer.encode(
                    e["cible"], truncation=True, add_special_tokens=False
                ),
            },
            "label": float(e["moy"]),
        }
