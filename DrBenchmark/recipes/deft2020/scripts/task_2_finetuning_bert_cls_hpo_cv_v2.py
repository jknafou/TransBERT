        ...
        res = {
            "input_ids": {
                ...
                "source": tokenizer.encode(
                    e["source"], truncation=True, add_special_tokens=False
                ),
                "text_1_ids": tokenizer.encode(
                    e["cible_1"], truncation=True, add_special_tokens=False
                ),
                "text_2_ids": tokenizer.encode(
                    e["cible_2"], truncation=True, add_special_tokens=False
                ),
                "text_3_ids": tokenizer.encode(
                    e["cible_3"], truncation=True, add_special_tokens=False
                ),
            },
            "label": str(e["correct_cible"] + 1),
        }
        return res
    ...
    class collate_fn:
        ...
        def __call__(self, features):
            ...
            for i in range(len(features)):
                if (
                    len(features[i]["input_ids"]["source"])
                    + len(features[i]["input_ids"]["text_1_ids"])
                    + len(features[i]["input_ids"]["text_2_ids"])
                    + len(features[i]["input_ids"]["text_3_ids"])
                    + 5
                    > args.max_position_embeddings
                ):
                    features[i]["input_ids"]["source"] = features[i]["input_ids"][
                        "source"
                    ...
                    features[i]["input_ids"]["text_3_ids"] = features[i]["input_ids"][
                        "text_3_ids"
                    ][: args.max_position_embeddings // 2 - 1]

                shuffled_labels_list = np.random.permutation(labels_list).tolist()
                label = shuffled_labels_list.index(features[i]["label"])
                token_ids = (
                    [self.tokenizer.cls_token_id]
                    + features[i]["input_ids"]["source"]
                    + [self.tokenizer.sep_token_id]
                    + features[i]["input_ids"][
                        "text_" + shuffled_labels_list[0] + "_ids"
                    ]
                    + [self.tokenizer.sep_token_id]
                    + features[i]["input_ids"][
                        "text_" + shuffled_labels_list[1] + "_ids"
                    ]
                    + [self.tokenizer.sep_token_id]
                    + features[i]["input_ids"][
                        "text_" + shuffled_labels_list[2] + "_ids"
                    ]
