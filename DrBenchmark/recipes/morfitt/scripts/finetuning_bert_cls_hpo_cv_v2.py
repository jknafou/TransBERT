...
def toLogits(predictions, threshold=THRESHOLD_VALUE):
    ...
    return y_pred, probs.detach().numpy()


def multi_label_metrics(y_pred, labels):
    y_true = labels
...
def compute_metrics(p: EvalPrediction):
    y_pred, prob = toLogits(
        p.predictions[0] if isinstance(p.predictions, tuple) else p.predictions
    )
    result = multi_label_metrics(y_pred=y_pred, labels=p.label_ids)
    return result
...
def main():
    ...
    labels_list = dataset.features["specialities"].feature.names

    # Remove duplicates
    seen = set()

    def filter_duplicate(example):
        if example["abstract"] in seen:
            return False
        seen.add(example["abstract"])
        return True

    dataset = dataset.filter(filter_duplicate)
    ...
    output_name = f"DrBenchmark-MORFITT-cls-{str(uuid.uuid4().hex)}_fold={args.fold}"
    ...
    predictions, probs = toLogits(predictions, THRESHOLD_VALUE)
