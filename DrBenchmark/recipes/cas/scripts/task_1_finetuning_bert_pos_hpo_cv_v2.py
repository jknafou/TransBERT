...
def main():
    ...
    pos_tags_list = dataset["pos_tags"]

    # Convert list of lists to tuple of tuples
    tokens_tuple = [tuple(tokens) for tokens in tokens_list]
    pos_tags_tuple = [tuple(pos_tags) for pos_tags in pos_tags_list]

    # Use a dictionary to count occurrences of each token list
    tokens_count = {}
    for tokens, pos_tags in zip(tokens_tuple, pos_tags_tuple):
        if tokens not in tokens_count:
            tokens_count[tokens] = []
        tokens_count[tokens].append(pos_tags)

    # Identify duplicates
    duplicates = {
        tokens: pos_tags
        for tokens, pos_tags in tokens_count.items()
        if len(pos_tags) > 1
    }

    #  Check if labels across duplicates are the same
    for tokens, pos_tags in duplicates.items():
        if len(set(pos_tags)) > 1:
            print(tokens, pos_tags)
    #  They always have the same labels -> we can remove duplicates without losing information
    ...
    # 3790 -> 3753 after removing duplicates
    ...
    output_name = f"DrBenchmark-CAS-{str(args.subset)}-{uuid.uuid4()}_fold={args.fold}"
